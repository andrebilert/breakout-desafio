...
let x = canvas.width / 2 //localização
let y = canvas.height - 30 //localização
let dx = 2 //velocidade
let dy = -2 //velocidade
...
//Transformando em função para utilizar dentro da função detectaColisão
function estadoBloco () {
    ...
    for (let l = 0; l < contagemDeBlocosEmLinha; l++) {
      ...
      if (bloco.estado === 1) {
        if (
          ...
          //Para aumentar a velocidade será necessário adicionar um nível como um contador, fiz uma função para definir o estado do bloco
          if (pontos == ((contagemDeBlocosEmLinha * contagemDeBlocosEmColuna) * nivel)) {
            ...
            dy = -2 - nivel
            nivel++
            //Mudando estado do bloco pra entrar no if do desenhaBloco
            estadoBloco()
...
//Adicionando o nivel na tela
function desenhaNivel() {
  context.font = '20px Arial'
  context.fillStyle = 'white'
  context.fillText(`Nível: ${nivel}`, 220, 30)
}

function desenhaVidas() {
  context.font = '20px Arial'
  context.fillStyle = 'lime'
  context.fillText(`Vidas: ${vidas}`, canvas.width - 75, 30)
}
  ...
  if (y + dy < raioDaBola) {
    ...
    if (x > raqueteX && x < raqueteX + larguraDaRaquete) {
      ...
      if (!vidas) {
        ...
        x = canvas.width / 2
        y = canvas.height - 30
        //caso queira manter a mesma velocidade atual ao perder vida, fazer dx = dx e dy = dy
        dx = 2
        dy = -2
